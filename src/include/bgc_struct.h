...
/* water state variables (including sums for sources and sinks) */
  typedef struct
  {
    ...
    ///////*added by Y.He Oct/14/15*////////
    double outflow_dummy_snk;	/* (kgH2O/m2) SUM of outflow_dummy */
...
/* soil and site constants */
  typedef struct
  {
    ...
    double obs_alpha;		/* (cm) vanGenueten Parameter, alpha */
    double obs_beta;		/* vanGenueten Parameter, beta */
    double obs_theta_s;		/* (%) soil saturation */
    double obs_theta_r;		/* (%) minimum soil water? */
    double obs_fc;		/* (%) field capacity */
} siteconst_struct;
...
/* structure for the photosynthesis routine */
  ...
  typedef struct
  {
    ...
    double daily_soil_resp;	/* kgC/m2/day  daily soil+root respiration, compare with observation, add by Y.He*/
