				...
				for (pday=244 ; pday<305 ; pday++)
				{
					if (south)
					{
						if (py==phenyears-1 && pday>181)
						{
							...
							phensoilt = metarr->obs_soilT[ndays-547+pday];
							//edited by Y. He Oct/29/14

							//phensoilt = metarr->tavg_ra[ndays-547+pday];
						}
						else
						{
							phensoilt = metarr->obs_soilT[py*365-182+pday];
							//edited by Y.He Oct/29/14
					...
					{
						phensoilt = metarr->obs_soilT[py*365+pday];
						//edited by Y.He Oct/29/14
				...
				for (pday=0 ; pday<365 ; pday++)
				{
					if (south)
					{
						if (py==0 && pday<182)
						{
							...
							phensoilt = metarr->obs_soilT[183+pday];
							//edited by Y.He Oct/29/14
