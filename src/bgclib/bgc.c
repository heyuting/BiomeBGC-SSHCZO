...
int
bgc (bgcin_struct * bgcin, bgcout_struct * bgcout, int mode)
{
...
//  printf("calling bgc()\n");//added by Y.He to test
  ...
  double annmaxplai, annet, annoutflow, anngpp, annnpp, annnbp, annprcp,
    anntavg;
  ...
  if (ok && dayout
    ...
    {
      bgc_printf (BV_ERROR,
		  "Error allocating for local daily output array in bgc()\n");
      ok = 0;
    }
  if (ok && ctrl.domonavg
    ...
    {
      bgc_printf (BV_ERROR,
		  "Error allocating for monthly average output array in bgc()\n");
      ok = 0;
    }
  if (ok && ctrl.doannavg
    ...
    {
      bgc_printf (BV_ERROR,
		  "Error allocating for annual average output array in bgc()\n");
      ok = 0;
    }
  if (ok && ctrl.doannual
    ...
    {
      bgc_printf (BV_ERROR,
		  "Error allocating for local annual output array in bgc()\n");
      ok = 0;
    }
  ...
  if (ok && !(output_map = (double **) malloc (NMAP * sizeof (double *))))
    {
      bgc_printf (BV_ERROR,
		  "Error allocating for output map in output_map_init()\n");
      ok = 0;
    }
  ...
  if (ok
      && output_map_init (output_map, &metv, &ws, &wf, &cs, &cf, &ns, &nf,
			  &phen, &epv, &psn_sun, &psn_shade, &summary))
    {
...
//      printf("done firstday\n");//added by Y.He to test
  ...
  do
      ...
      for (simyr = 0; ok && simyr < tmpyears; simyr++)
	{
	  ...
	  if (metyr == ctrl.metyears)
		...
		{
		  if (ctrl.onscreen)
		    bgc_printf (BV_DETAIL,
				"Resetting met data for cyclic input\n");
		}
	      if (mode == MODE_SPINUP)
		{
		  bgc_printf (BV_DIAG,
	  ...
	  if (mode == MODE_MODEL)
	      ...
	      else
		{
		  ...
		  if (co2.varco2 == 1)
		    metv.co2 = get_co2 (&co2, (ctrl.simstartyear + simyr));
		  bgc_printf (BV_DIAG, "CO2 val: %lf Year: %i\n", metv.co2,
			      (ctrl.simstartyear + simyr));
		  if (metv.co2 < -999)
		    {
		      bgc_printf (BV_ERROR,
				  "Error finding CO2 value for year: %i\n",
				  (ctrl.simstartyear + simyr));
		      return (EXIT_FAILURE);
		    }
	      ...
	      if (bgcin->ndepctrl.varndep && mode == MODE_MODEL)
		{
		  ...
		  daily_ndep =
		    get_ndep (&bgcin->ndepctrl, (ctrl.simstartyear + simyr));
		  if (daily_ndep < -999)
		    {
		      bgc_printf (BV_ERROR,
				  "Error finding NDEP for year: %i\n",
				  (ctrl.simstartyear + simyr));
		      return (EXIT_FAILURE);
		    }
		  else
	  ...
	  for (yday = 0; ok && yday < 365; yday++)
		...
		{
		  bgc_printf (BV_ERROR,
			      "Error in call to precision_control() from bgc()\n");
		  ok = 0;
		}
		...
		epv.ytd_maxplai = epv.proj_lai;
	      ...
	      if (ok && metv.prcp
		  && prcp_route (&metv, epc.int_coef, epv.all_lai, &wf))
		{
		...
		{
		  bgc_printf (BV_ERROR,
			      "Error in baresoil_evap() from bgc()\n");
		  ok = 0;
		}
	      ...
	      if (ok
		  && soilpsi (&sitec, ws.soilWobs, &epv.psi, &epv.vwc, &ws))
		{
		...
		{
		  if (ok
		      && total_photosynthesis (&metv, &epc, &epv, &cf,
					       &psn_sun, &psn_shade))
		    {
		      bgc_printf (BV_ERROR,
				  "Error in total_photosynthesis() from bgc()\n");
		      ok = 0;
		    }
	      ...
	      if (ok && outflow (&sitec, &ws, &wf))
		{
	      ...
	      if (ok
		  && decomp (metv.soilTobs, &epc, &epv, &sitec, &cs, &cf, &ns,
			     &nf, &nt))
		{
	      ...
	      if (mode == MODE_MODEL)
		{
		  if (ok && daily_allocation (&cf, &cs, &nf, &ns, &epc, &epv, &nt, &ws, &phen, 1.0, MODE_MODEL))	//add &ws by Yuting, 06/24/2014, 11/29/2014
		    {
		      bgc_printf (BV_ERROR,
				  "Error in daily_allocation() from bgc.c\n");
		      ok = 0;
		    }
		}
	      else if (mode == MODE_SPINUP)
		{
		  ...
		  if (!steady1 && rising && metcycle == 0)
		    {
		      if (ok
			  && daily_allocation (&cf, &cs, &nf, &ns, &epc, &epv,
					       &nt, &ws, &phen, naddfrac,
					       MODE_SPINUP))
			{
			  bgc_printf (BV_ERROR,
	      ...
	      if (ok && annual_alloc)
		{
		  if (ok && annual_rates (&epc, &epv))
		    {
		      bgc_printf (BV_ERROR,
				  "Error in annual_rates() from bgc()\n");
		      ok = 0;
		    }
		...
		{
		  bgc_printf (BV_ERROR,
			      "Error in daily_growth_resp() from bgc.c\n");
		  ok = 0;
		}
	      ...
	      if (ok && daily_water_state_update (&wf, &ws,&sitec))
		{
		  bgc_printf (BV_ERROR,
			      "Error in daily_water_state_update() from bgc()\n");
		  ok = 0;
		}
		...
		{
		  bgc_printf (BV_ERROR,
			      "Error in daily_carbon_state_update() from bgc()\n");
		  ok = 0;
		}
		...
		{
		  bgc_printf (BV_ERROR,
			      "Error in daily_nitrogen_state_update() from bgc()\n");
		  ok = 0;
		}
		...
		{
		  bgc_printf (BV_ERROR,
			      "Error in check_water_balance() from bgc()\n");
		...
		{
		  bgc_printf (BV_ERROR,
			      "Error in check_carbon_balance() from bgc()\n");
		  bgc_printf (BV_ERROR, "%d\n", metday);
		  ok = 0;
		}
		...
		{
		  bgc_printf (BV_ERROR,
			      "Error in check_nitrogen_balance() from bgc()\n");
		  bgc_printf (BV_ERROR, "%d\n", metday);
		  ok = 0;
		}
	      ...
	      if (ok && ctrl.dodaily)
		{
		  ...
		  if (fwrite
		      (dayarr, sizeof (float), ctrl.ndayout,
		       bgcout->dayout.ptr) != (size_t) ctrl.ndayout)
		    {
		      bgc_printf (BV_ERROR,
				  "Error writing to %s: simyear = %d, simday = %d\n",
		  ...
		  if (ok && bgcout->bgc_ascii)
		    {
		      ...
		      output_ascii (dayarr, ctrl.ndayout,
				    bgcout->dayoutascii.ptr);
	      ...
	      if (ctrl.domonavg)
		{
		  ...
		  for (outv = 0; outv < ctrl.ndayout; outv++)
		      ...
		      switch (ctrl.daycodes[outv])
			{
			  /* Leaf area index */
			case 545:
			  if (dayarr[outv] > monmaxlai)
			    monmaxlai = dayarr[outv];
			  break;
			}
		    }
		  ...
		  if (yday == endday[curmonth])
		      ...
		      for (outv = 0; outv < ctrl.ndayout; outv++)
			{
		      ...
		      /* write to file */
		      if (fwrite
			  (monavgarr, sizeof (float), ctrl.ndayout,
			   bgcout->monavgout.ptr) != (size_t) ctrl.ndayout)
			{
			  bgc_printf (BV_ERROR,
				      "Error writing to %s: simyear = %d, simday = %d\n",
		      ...
		      if (ok && bgcout->bgc_ascii)
			{
			  output_ascii (monavgarr, ctrl.ndayout,
					bgcout->monoutascii.ptr);
	      ...
	      if (ctrl.doannavg)
		{
		  ...
		  for (outv = 0; outv < ctrl.ndayout; outv++)
		      ...
		      switch (ctrl.daycodes[outv])
			{
			  /* Leaf area index */
			case 545:
			  if (dayarr[outv] > annmaxplai)
			    annmaxplai = dayarr[outv];
			  break;
			}
		    }
		  ...
		  if (yday == 364)
		      ...
		      for (outv = 0; outv < ctrl.ndayout; outv++)
			{
		      ...
		      /* write to file */
		      if (fwrite
			  (annavgarr, sizeof (float), ctrl.ndayout,
			   bgcout->annavgout.ptr) != (size_t) ctrl.ndayout)
			{
			  bgc_printf (BV_ERROR,
				      "Error writing to %s: simyear = %d, simday = %d\n",
	      ...
	      if (mode == MODE_MODEL)
		{
		  ...
		  if (epv.proj_lai > (double) annmaxlai)
		    annmaxlai = (float) epv.proj_lai;
	  ...
	  if (ok && ctrl.doannual)
	      ...
	      for (outv = 0; outv < ctrl.nannout; outv++)
		{
	      ...
	      if (fwrite
		  (annarr, sizeof (float), ctrl.nannout,
		   bgcout->annout.ptr) != (size_t) ctrl.nannout)
		{
		  bgc_printf (BV_ERROR,
			      "Error writing to %s: simyear = %d, simday = %d\n",
	      ...
	      if (ok && bgcout->bgc_ascii)
		{
		  ...
		  output_ascii (annarr, ctrl.nannout,
				bgcout->annoutascii.ptr);
	  ...
	  if (mode == MODE_MODEL && bgcout->bgc_ascii)
	    {
	      ...
	      fprintf (bgcout->anntext.ptr,
		       "%6d%10.1f%10.1f%10.1f%10.1f%10.1f%10.1f%10.1f%10.1f\n",
		       ctrl.simstartyear + simyr, annprcp, anntavg, annmaxlai,
		       annet, annoutflow, annnpp, annnbp, anngpp);
	    }
      ...
      if (mode == MODE_SPINUP)
	{
	  ...
	  if (!steady1 && metcycle == 2)
	    {
	      ...
	      bgc_printf (BV_DIAG,
			  "metcycle = %d tally1 = %lf tally2 = %lf pdif = %lf\n\n",
			  metcycle, tally1, tally2, t1);
	      if (steady1)
		bgc_printf (BV_DIAG, "SWITCH\n\n");
	  ...
	  /* second block is after supplemental N turned off */
	      ...
	      if (t1 > SPINUP_TOLERANCE)
		{
	      ...
	      bgc_printf (BV_DIAG,
			  "spinyears = %d rising = %d steady2 = %d\n",
			  spinyears, rising, steady2);
	      bgc_printf (BV_DIAG,
			  "metcycle = %d tally1 = %lf tally2 = %lf pdif = %lf\n\n",
			  metcycle, tally1, tally2, t1);
  ...
  if (ok)
	...
	{
	  bgc_printf (BV_ERROR,
		      "Error in call to restart_output() from bgc()\n");
	  ok = 0;
	}
